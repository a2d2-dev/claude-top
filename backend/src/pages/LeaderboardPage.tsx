...
interface LeaderboardPageProps {
  rows: LeaderboardRow[];
  period: string;
  /** 默认激活的 tab：'claude' / 'codex' / 'about'，默认 'about' */
  defaultTab?: 'claude' | 'codex' | 'about';
  /** 数据来源，与 defaultTab 一致，决定哪个榜单有真实数据。 */
  source?: 'claude' | 'codex';
}
...
/* Codex tab 绿色激活态 */
.tab-btn.active-codex { color: #10A37F; border-bottom-color: #10A37F; }

/* Codex 榜单绿色主题覆盖 */
#tab-codex .cost-val { color: #10A37F; }
#tab-codex .lb-period { color: #10A37F; background: rgba(16,163,127,0.1); border-color: rgba(16,163,127,0.3); }
...
const pageScript = `
(function() {
  // ── 统一 Tab 切换（claude / codex / about）──
  // 切换 leaderboard tab 时如果 source 变了则整页跳转（重新加载正确数据）。
  function showTab(id) {
    var currentSource = new URL(location.href).searchParams.get('source') || 'claude';
    if ((id === 'claude' || id === 'codex') && currentSource !== id) {
      // source 变了：重新加载带正确 ?source= 的 URL，保留 /leaderboard 路径
      var url = new URL(location.href);
      var base = url.pathname.startsWith('/leaderboard') ? url.pathname : '/leaderboard';
      url.pathname = base;
    ...
    // 同 source 或切到 about：直接显示/隐藏面板
    document.querySelectorAll('.tab-panel').forEach(function(p) { p.classList.remove('active'); });
    document.querySelectorAll('.tab-btn').forEach(function(b) {
      b.classList.remove('active', 'active-codex');
    });
    var panel = document.getElementById('tab-' + id);
    var btn   = document.querySelector('[data-tab="' + id + '"]');
    if (panel) panel.classList.add('active');
    if (btn) {
      if (id === 'codex') btn.classList.add('active-codex');
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // 初始 tab：URL ?source= 或 #about 优先，否则用服务端默认值
    var urlSource = new URL(location.href).searchParams.get('source');
    var hash      = location.hash.replace('#', '');
    var validTabs = ['claude', 'codex', 'about'];
    var initTab;
    if (hash === 'about') initTab = 'about';
    else if (urlSource === 'codex') initTab = 'codex';
    else if (urlSource === 'claude') initTab = 'claude';
    else initTab = __DEFAULT_TAB__;
    // 直接设置面板可见，不触发重载逻辑（页面已是对应 source）
    document.querySelectorAll('.tab-panel').forEach(function(p) { p.classList.remove('active'); });
    document.querySelectorAll('.tab-btn').forEach(function(b) { b.classList.remove('active', 'active-codex'); });
    var panel = document.getElementById('tab-' + initTab);
    var btn   = document.querySelector('[data-tab="' + initTab + '"]');
    if (panel) panel.classList.add('active');
    if (btn) {
      if (initTab === 'codex') btn.classList.add('active-codex');
      else btn.classList.add('active');
    }
...
export const LeaderboardPage = ({ rows, period, defaultTab = 'about', source = 'claude' }: LeaderboardPageProps) => (
  ...
  >
    ...
    {/* ── Tab 导航栏（三 tab 扁平结构）── */}
    <div class="page-tabs">
      <button class="tab-btn" data-tab="claude" onclick="showTab('claude')">
        🤖&nbsp;Claude Code
      </button>
      <button class="tab-btn" data-tab="codex" onclick="showTab('codex')">
        ✦&nbsp;Codex CLI
      </button>
